    ...
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($student_id) || empty($fullname) || empty($email)) {
            ...
            if ($check_stmt->get_result()->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    $_SESSION['success_message'] = "Student '" . htmlspecialchars($fullname, ENT_QUOTES, 'UTF-8') . "' has been added successfully!";
    ...
    <div class="form-card">
        ...
        <div class="form-body">
            ...
            <form method="POST" action="add_student.php">
                ...
                <button type="submit" name="add" class="btn-full"><i class="fas fa-check"></i> Add Student</button>
            </form>
        </div>
    </div>
</div>
